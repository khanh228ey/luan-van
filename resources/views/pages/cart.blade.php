...
@section('content')
            ...
            <form id="cartForm" action="{{ route('checkout.page') }}" method="POST">
                ...
                <div class="row">
                    <div class="col-12 text-center">
                        ...
                        <p class="mb-5"><span class="primary-color">{{ $cartUser->count() }}</span> sản phẩm trong giỏ hàng
                        </p>
                        <table id="cart" class="table table-condensed">
                            ...
                            <tbody>
                                @foreach ($cartUser as $item)
                                    <tr>
                                        <td>
                                            <input type="checkbox" class="cart-checkbox" name="cart_item_ids[]"
                                                value="{{ $item->id }}">
                                            <input type="hidden" name="product_ids[{{ $item->id }}]"
                                                value="{{ $item->product->id }}">
                                            <input type="hidden" name="product_detail_ids[{{ $item->id }}]"
                                                value="{{ $item->productDetail->id }}">
                                            <input type="hidden" class="cart-qty-hidden" name="quantities[{{ $item->id }}]"
                                                value="{{ $item->quantity }}">
                                        </td>
                                        <td data-th="Product">
                                            <div class="row">
                                        ...
                                        <td class="actions" data-th="">
                                            <div class="text-right">
                                                <form action="{{ route('cart.delete') }}" method="POST" style="display:inline;">
                                                    ...
                                                    <button type="submit" class="btn btn-white btn-md mb-2"><i class="fas fa-trash"></i>
                                                    </button>
                                                </form>
                                            </div>
                                        </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Đồng bộ số lượng khi thay đổi input
            ...
            // Khi submit, chỉ gửi số lượng của sản phẩm được chọn
            ...
            document.querySelectorAll('.cart-qty-input').forEach(function(input) {
                input.addEventListener('change', function() {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Có thể cập nhật tổng tiền ở đây nếu muốn
